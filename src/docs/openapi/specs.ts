 ...
 */
export function getHeaderParams(route: IRoute): ParameterObject[] {
  const headers: ParameterObject[] = route.params
    .filter((p) => p.type === 'header')
    .map((headerMeta) => {
      const schema = getParamSchema(headerMeta) as SchemaObject;
      return {
  ...
  if (headersMeta) {
    const schema = getParamSchema(headersMeta) as ReferenceObject;
    headers.push({
      in: 'header',
 ...
 */
export function getPathParams(route: IRoute): ParameterObject[] {
...
export function getQueryParams(
  ...
  const queries: ParameterObject[] = route.params
    .filter((p) => p.type === 'query')
    .map((queryMeta) => {
      const schema = getParamSchema(queryMeta) as SchemaObject;
      return {
  ...
  if (queriesMeta) {
    const paramSchema = getParamSchema(queriesMeta) as ReferenceObject;
    ...
    for (const [name, schema] of Object.entries(
      ...
      queries.push({
        ...
        schema: schema as Partial<OpenAPIObject>,
      });
    }
  }
  return queries;
 ...
 */
export function getRequestBody(route: IRoute): RequestBodyObject | void {
  const bodyParamMetas = route.params.filter((d) => d.type === 'body-param');
  const bodyParamsSchema: SchemaObject | null =
  ...
  if (bodyMeta) {
    ...
    const { $ref } =
      'items' in bodySchema && bodySchema?.items
        ? (bodySchema.items as ReferenceObject)
        : (bodySchema as ReferenceObject);
 ...
 */
export function getResponses(route: IRoute): ResponsesObject {
...
function getParamSchema(
  ...
  const type: (() => unknown) | unknown =
    Reflect.getMetadata('design:paramtypes', object, method)?.[index] ||
    Reflect.getMetadata('custom:annotation', object, method) ||
    null;
